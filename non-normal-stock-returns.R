...
dbClearResult(stmt)
ggsave('ibex35-daily-rate.png', p + theme(text = element_text(size=6)) + geom_text(hjust='right', data=subset(ibex35, rate %in% c(min(rate), max(rate))), size=1.5, aes(x=date, y=rate, label=paste(date, ': ', round(exp(rate)*100-100, 2), '% ', sep=''))), 'png', 'analysis/static/analysis', height=55, units=c("mm"))
